...
void serve_static_file(const int fd, const char *file_path) {
    ...
    snprintf(header, sizeof(header), "HTTP/1.1 200 OK\r\nContent-Length: %ld\r\nContent-Type: %s\r\nConnection: close\r\n\r\n",
        file_stat.st_size,
        (endswith(file_path, ".html") ? "text/html" : "text/plain"));
...
void *worker_thread(void *arg) {
    while (1) {
        ...
        char log_msg[LOG_BUFFER];
        snprintf(log_msg, sizeof(log_msg), "Request: %s on %s", method, path);
        log_event(log_msg);
...
int main() {
    ...
    char log_msg[LOG_BUFFER];
    snprintf(log_msg, sizeof(log_msg), "Server starting on :%d", PORT);
    log_event(log_msg);
